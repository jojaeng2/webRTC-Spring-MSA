...
public class HttpApiController {
    ...
    public User postFindUserByEmail(String email) {
        try {
            ResponseEntity<String> response = postRequest("http://localhost:8081/api/v1/webrtc/user", new FindUserByEmailRequest(email));
    ...
    public User postSaveUser(CreateUserRequest request) {
        String response = postRequest("http://localhost:8081/api/v1/webrtc/register", request).getBody();
