...
public class ChannelListRepositoryImpl implements ChannelListRepository {
  ...
  public List<Channel> findAnyChannels(int idx, String type) {
    return em.createQuery("select c from Channel c order by c.currentParticipants " + type,
            Channel.class)
  ...
  public List<Channel> findMyChannels(String userId, int idx, String type) {
    return em.createQuery(
            "select c from Channel c join c.channelUsers where USERS_ID = :user_id order by c.currentParticipants "
                + type, Channel.class)
  ...
  public List<Channel> findChannelsByHashName(HashTag hashTag, int idx, String type) {
    return em.createQuery(
            "select c from Channel c join c.channelHashTags where hashtag_id = :hashtag_id order by c.currentParticipants "
                + type, Channel.class)
