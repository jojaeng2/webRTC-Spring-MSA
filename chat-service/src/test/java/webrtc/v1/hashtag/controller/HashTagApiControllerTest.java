...
public class HashTagApiControllerTest {
  ...
  public void setup(RestDocumentationContextProvider restDocumentationContextProvider)
    ...
    // jwt token 생성
    doReturn(
        new org.springframework.security.core.userdetails.User(email1, password, new ArrayList<>()))
    ...
    mockMvc.perform(RestDocumentationRequestBuilders.get(
        ...
        .andDo(document("return-hashtagchannels-success",
            ...
            responseFields(
                ...
                fieldWithPath("channels[].limitParticipants").type(NUMBER)
                    .description("채널에 참여할 수있는 제한인원"),
                fieldWithPath("channels[].currentParticipants").type(NUMBER)
                    .description("채널에 현재 참여중인 인원"),
                fieldWithPath("channels[].timeToLive").type(NUMBER).description("채널이 삭제되기까지 남은 시간"),
                fieldWithPath("channels[].channelHashTags").type(ARRAY)
                    .description("채널에 사용된 해시태그들"),
                fieldWithPath("channels[].channelHashTags[].hashTag").type(OBJECT)
                    .description("채널에 사용된 해시태그"),
                fieldWithPath("channels[].channelHashTags[].hashTag.tagName").type(STRING)
                    .description("해시태그 이름"),
